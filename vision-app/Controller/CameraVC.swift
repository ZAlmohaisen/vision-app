...
//  vision-app
//
//  Created by Ziyad almohisen on 18/06/2019.
...
class CameraVC: UIViewController {
    ...
    @IBOutlet weak var confiedenceLbl: UILabel!
    ...
    func resultsMethod(request: VNRequest, error: Error?) {
        ...
        for classification in result {
            if classification.confidence < 0.5 {
                ...
                self.identLbl.text = unknownObjectMessage
                synthesizeSpeech(fromString: unknownObjectMessage)
                self.confiedenceLbl.text = ""
                break
            } else {
                let identification = classification.identifier
                let confidence = Int(classification.confidence * 100)
                self.identLbl.text = identification
                self.confiedenceLbl.text = "CONFIDENCE \(confidence)%"
